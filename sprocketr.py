...
'''
def getArcLengthMinor(dia_big, dia_little, CTC):
    arc_little = 2.0 * 3.14 * (.5 * dia_little) * ((2* arccos(
''' 
'''
...
def getSlack(picthDiameter, picthDiametertwo, CTC):
    A = .13333/2
    hypotenuseLength = math.sqrt((.5*picthDiametertwo - .5*picthDiameter)**2 + (.5*picthDiameter + .5*picthDiametertwo + (A*2))**2)
    length = 2*hypotenuseLength + (.5 * picthDiameter * 3.14159) + (.5 * picthDiametertwo * 3.14159)
    '''
...
def getCenterToCenterList(picthDiameter, picthDiametertwo, P, A,dslack,tCTC):
    CTC=tCTC

    hypotenuseLength = math.sqrt(((.5*picthDiametertwo - .5*picthDiameter)**2 + (CTC)**2))
    length = 2*hypotenuseLength + (.5 * picthDiameter * 3.14159) + (.5 * picthDiametertwo * 3.14159)
    ...
    for i in range(0, 50):
        length = length + pitch
        hypotenuseLength = .5*(length- (.5*picthDiameter*3.14159) - (.5*picthDiametertwo*3.14159))
        CTC = math.sqrt(hypotenuseLength**2 - ((.5*picthDiametertwo - .5*picthDiameter)**2)+dslack)
        links=(length)/(pitch)
        CTClist.append((CTC,length,links))
    return CTClist
...
def findSideRatio(ratio,side,CTC,dslack,errorA):
    ...
    findAllSetsInRatio(ratio,CTC,dslack,errorA)
    #sprocketArray = sorted(sprocketArray, key= sprocketArray(terror))
    return
...
def getFirstRatio(tratio):
    ...
    for ratioScaler in range(1,28):
        if (.5*float(little*ratioScaler)>=6) and (big*ratioScaler<56):
...
def addAllSetsInCTCRange(tratio,fCTC,dslack,error):
    ...
    CTC = fCTC*0.0393701
    CTCArray = getCenterToCenterList(getPitchDiameter(littleTeeth),getPitchDiameter(bigTeeth),pitch,.13/2,.1,CTC)
    for x in range(0,6):
        print(CTC)
        ...
        if terror >error:
            works=False
        infoset = (bigTeeth,littleTeeth,big,little,CTC,length,links,slack,terror,works,getPitchDiameter(bigTeeth)+getAmplitude(bigTeeth),getPitchDiameter(littleTeeth)+getAmplitude(littleTeeth))
        #print(infoset)
...
def run(origo):
    ...
    for t in range(0, resolution):
        ...
        myArrayVerts.append((x,y,z+thickness+zModifier))
...
def processArray(uarg):
    ...
    if refinetype=='ratio':
        ...
        (littleTeeth,bigTeeth,little,big,fCTC,length,links,slack,terror,works,bigDia,littleDia)=best
        options=[]
        for x in range(0,5):                
            sprocket={}
            ...
            sprocket["length"]=round(length,3)
            ...
            sprocket["distFromDesiredSlack"]=(uperror * length)-terror
    ...
    if refinetype=='c2c':
        ...
        for x in range(0,5):                
            ...
            sprocket["length"]=round(length*25.4,1)
            sprocket["links"]=links
            sprocket["slack"]=round(slack,3)
            sprocket["error"]=terror
            sprocket["err_acceptable"]=works
        ...
        if left == 0:
            ...
            #sprocketArray.append(infoset)
            (littleTeeth,bigTeeth,little,big,fCTC,length,links,slack,terror,works,bigDia,littleDia)=infoset
